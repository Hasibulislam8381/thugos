...
<head>
    ...
    <link rel="stylesheet" href="{{ static_asset('assets/css/styleim.css?v1')  }}">
    <link rel="stylesheet" href="{{ static_asset('assets/css/responsivehim.css?v1')  }}">
    <link rel="stylesheet" href="{{ static_asset('assets/css/responsveim.css?v1')  }}">
    <link rel="stylesheet" href="{{ static_asset('assets/css/owl.carousel.min.css')  }}">
    <link rel="stylesheet" href="{{ static_asset('assets/css/owl.theme.default.min.css')  }}">
...
s.parentNode.insertBefore(t,s)}(window, document,'script',
'https://connect.facebook.net/en_US/fbevents.js');
fbq('init', '1220219705946833');
fbq('track', 'PageView');
</script>
<noscript><img height="1" width="1" style="display:none"
src="https://www.facebook.com/tr?id=1220219705946833&ev=PageView&noscript=1"
/></noscript>
<!--End-->
...
})(window,document,'script','dataLayer','GTM-N4QL9HV8');</script>
<!-- End Google-->
...
<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-N4QL9HV8"
...
<!-- End Google Tag Manager-->
    ...
    <script async defer crossorigin="anonymous" src="https://connect.facebook.net/en_US/sdk.js#xfbml=1&version=v11.0" nonce="YOUR_NONCE_CODE"></script>
...
function search(input) {
    ...
    if (searchKey.length > 0) {
        ...
        function notifyMe(id){
            ...
            $('#addToCart').modal();
            $('.c-preloader').show();
            $.post('{{ route('cart.showNotifyModal') }}', {_token: AIZ.data.csrf, id:id}, function(data){
                $('.c-preloader').hide();
                $('#addToCart-modal-body').html(data);
                AIZ.plugins.slickCarousel();
                AIZ.plugins.zoom();
                ...
                $.ajax({
                   ...
                   success: function(data){
                       ...
                       if(data.offer_discount > 0){
                        $('.discount_text').html('(Discount: ');
                       $('.offer_discount').html('৳'+data.offer_discount + ')');
                       }else{
                        $('.discount_text').html('');
                        $('.offer_discount').html('');
                       }
        ...
        function buyNow_temp(product_id){
            if(checkAddToCartValidity()) {
                // $('#addToCart-modal-body').html(null);
                // $('#addToCart').modal();
                // $('.c-preloader').show();
                $.ajax({
                   ...
                   success: function(data){
                       if(data.status == 1){
                            ...
                            // $('#addToCart-modal-body').html(data.modal_view);
                            updateNavCart(data.nav_cart_view,data.cart_count,data.nav_side_view,data.nav_side_price);
        ...
        function buyNow(){
            if(checkAddToCartValidity()) {
                ...
                $('#addToCart').modal();
                $('.c-preloader').show();
                var product_id = $('#product_box_id').val();
                $.ajax({
                   type:"POST",
                   url: '{{ route('cart.addToCart') }}',
                   data:$('#option-choice-form').serializeArray(),
