      ...
      <div class="row mobile_row">
         <div class="col-xl-6 col-lg-6">
            <div class="bg-white shadow-sm rounded p-3">
               <div class="sticky-top z-3 row gutters-10 grid_display">
                  {{-- <-- OPENED HERE --}}
                  ...
                  <div class="col order-2 order-md-2">
                     <div class="aiz-carousel product-gallery" data-nav-for='.product-gallery-thumb' data-fade='true' data-auto-height='true'>
                        ...
                        @if ($stock->image != null)
                        <div class="carousel-box img-zoom rounded d-flex">
                           <img class="img-fluid lazyload m-auto"
                              src="{{ static_asset('assets/img/placeholder.jpg') }}"
                  ...
                  <div class="col-12 col-md-auto w-md-80px w-527 order-1 order-md-12 mt-3 mt-md-0">
                     <div class="aiz-carousel product-gallery-thumb"
                        data-items='5'
               ...
               {{-- <-- FIXED: This closes sticky-top row --}}
            </div>
         </div>
         <div class="col-md-6">
            <div class="bg-white shadow-sm rounded p-3 min_width_698">
               <div  class="text-left pl_30">
                  ...
                  <div class="col-lg-12">
                     <div class="row align-items-center justify-content-between ">
                        <div class="d-flex">
                           ...
                           @if (discount_in_percentage($detailedProduct) > 0) <span class="badge-custom offer_text">{{ discount_in_percentage($detailedProduct) }}% {{ translate('OFF') }}</span> @endif @else 
                  ...
                  <div class="row mobile_row no-gutters mt-4">
                     <div class="col-sm-2">
                  ...
                  <form id="option-choice-form">
                     @csrf <input type="hidden" name="id" value="{{ $detailedProduct->id }}">
                     ...
                     <div class="row mobile_row no-gutters align-items-center my-2">
                        ...
                        <div class="col-md-9 col-sm-9 col-9">
                           <div class="aiz-radio-inline d-flex flex-wrap align-items-center">
                              ...
                              <img src="{{ uploaded_asset($firstImages[$key]) }}" width="40" height="40" alt="">
                              </span>
                              @endif
                              </span>
                              </label>
                     ...
                     <div class="row no-gutters align-items-center my-2 mt-3">
                        <!-- Type Label -->
                        <div class="col-md-3 col-sm-3 col-3">
                            <div class="select_color">Select Type:</div>
                        </div>
                     ...
                     @if ($qty >= 1) 
                     <div class="row mobile_row no-gutters mobile_pl_16">
                     ...
                     @endif 
                     <div class="row  no-gutters pb-2 d-none mobile_row mobile_pl_16" id="chosen_price_div">
                  ...
                  <div class="d-table width-100 mt-3">
                     <div class="d-table-cell">
                        ...
                        Auth::user()->affiliate_user->status) @php if(Auth::check()){ if(Auth::user()->referral_code == null){ Auth::user()->referral_code = substr(Auth::user()->id.Str::random(10), 0, 10); Auth::user()->save(); } $referral_code = Auth::user()->referral_code; $referral_code_url = URL::to('/product').'/'.$detailedProduct->slug."?product_referral_code=$referral_code"; } @endphp 
                        <div>
                  ...
                  <li>
                     ...
                     <div class="answer">
                        <div class="p-4 mobile-p-0">
                           ...
                           @endif @if (Auth::check()) @php $commentable = false; @endphp @foreach ($detailedProduct->orderDetails as $key => $orderDetail) @if (
                           $orderDetail->order != null &&
                           $orderDetail->order->user_id == Auth::user()->id &&
                           $orderDetail->delivery_status == 'delivered' &&
                           \App\Models\Review::where('user_id', Auth::user()->id)->where('product_id', $detailedProduct->id)->first() == null) @php $commentable = true; @endphp @endif @endforeach @if ($commentable) 
                           <div class="pt-4">
                              ...
                              <form class="form-default" role="form" action="{{ route('reviews.store') }}" method="POST">
                                 @csrf <input type="hidden" name="product_id" value="{{ $detailedProduct->id }}">
                                 <div class="row">
                                    <div class="col-md-6">
...
{{-- <script>
    // Variant images mapped by full variant string (e.g., Red-M)
    const variantImages = @json(
        $detailedProduct->stocks->filter(fn($s) => $s->image)->mapWithKeys(fn($s) => [
            $s->variant => uploaded_asset($s->image)
        ])
    );

    document.addEventListener('DOMContentLoaded', function () {
        const mainCarousel = $('.product-gallery');
        const thumbCarousel = $('.product-gallery-thumb');

        // Listen only to color changes for image update
        document.querySelectorAll('input[type="radio"][name="color"]').forEach(input => {
            input.addEventListener('change', updateVariantImage);
        });

        // Still listen to all attribute changes for other logic (e.g., price update, stock update)
        document.querySelectorAll('input[type="radio"][name^="attribute_id_"]').forEach(input => {
            input.addEventListener('change', function () {
                // You can handle price/stock updates here if needed
            });
        });
        ...
        function updateVariantImage() {
            ...
            // Add selected color first
            const selectedColor = document.querySelector('input[name="color"]:checked');
            if (selectedColor) {
                variantParts.push(selectedColor.value);
            }

            // Add other selected attributes
            document.querySelectorAll('input[type="radio"][name^="attribute_id_"]:checked').forEach(input => {
                variantParts.push(input.value);
            });

            const selectedVariant = variantParts.join('-');

            if (variantImages[selectedVariant]) {
                const imageSrc = variantImages[selectedVariant];

                // Remove previously added variant image
                const existingIndex = mainCarousel.find('.variant-added').closest('.slick-slide').data('slick-index');
                if (typeof existingIndex !== 'undefined') {
                    mainCarousel.slick('slickRemove', existingIndex);
                    thumbCarousel.slick('slickRemove', existingIndex);
                }

                // Add new variant image
                mainCarousel.slick('slickAdd', `
                    <div class="carousel-box img-zoom rounded variant-added">
                        <img class="img-fluid m-auto" src="${imageSrc}" alt="">
                    </div>
                `);

                thumbCarousel.slick('slickAdd', `
                    <div class="carousel-box c-pointer m-1 rounded variant-added">
                        <img class="mw-100 size-50px mx-auto border" src="${imageSrc}" alt="">
                    </div>
                `);

                const newIndex = mainCarousel.find('.slick-slide').length - 1;
                mainCarousel.slick('slickGoTo', newIndex);
            }
        }
    });
</script> --}}
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const firstMainImg = mainCarousel.find('.carousel-box img').first();
        const firstThumbImg = thumbCarousel.find('.carousel-box img').first();
        ...
        function updateVariantImage() {
            ...
            if (variantImages[selectedVariant]) {
                ...
                // Replace src attributes directly (no slick re-render)
                firstMainImg.attr('src', imageSrc).removeAttr('data-src').removeClass('lazyload');
                firstThumbImg.attr('src', imageSrc).removeAttr('data-src').removeClass('lazyload');

                // Optional: jump to first slide if not already
                mainCarousel.slick('slickGoTo', 0);
            }
        }
    });
</script>
