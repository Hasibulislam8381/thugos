    ...
    public function collection(Collection $rows) {
        ...
        if($canImport) {
            foreach ($rows as $row) {
                ...
                $productId = Product::updateOrCreate(['id' => $product_id],[
                            'name' => $row['name'],
                            ...
                            'thumbnail_img' => $this->downloadThumbnail($row['product_image']),
                            'photos' => $this->downloadThumbnail($row['product_image']),
                ...
                if($row->has('id')){
                    ProductStock::updateOrCreate(['product_id' => $product_id],[
                ...
                flash(translate('Products imported successfully'))->success();
                    ...
                    if(str_contains($ex->getMessage(), "Column 'category_id'")){
                        flash("Category Id reuired")->warning();
                    }elseif(str_contains($ex->getMessage(), "Column 'qty'")){
                        flash("Stock is reuired")->warning();
                    }else{
                        flash(translate($ex->getMessage()))->warning();
                    }
